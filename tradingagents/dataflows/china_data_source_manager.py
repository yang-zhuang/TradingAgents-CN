...
class DataSourceManager:
    ...
    def get_stock_data(self, symbol: str, start_date: str = None, end_date: str = None, period: str = "daily") -> str:
        ...
        try:
            ...
            duration = time.time() - start_time
            logger.error(f"❌ [数据获取] 异常失败: {e}", exc_info=True)
    ...
    def get_fundamentals_data(self, symbol: str) -> str:
        ...
        try:
            ...
            duration = time.time() - start_time
            logger.error(f"❌ [数据来源: {self.current_source.value}异常] 获取基本面数据失败: {symbol} - {e}", exc_info=True)
            return self._try_fallback_fundamentals(symbol)
    ...
    def get_news_data(self, symbol: str = None, hours_back: int = 24, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            duration = time.time() - start_time
            logger.error(f"❌ [数据来源: {self.current_source.value}异常] 获取新闻数据失败: {symbol or '市场新闻'} - {e}", exc_info=True)
            return self._try_fallback_news(symbol, hours_back, limit)
    ...
    def _get_tushare_data(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> str:
        ...
        try:
            # 1. 先尝试从缓存获取
            cached_data = self.cache_manager.get_cached_data(symbol, start_date, end_date, max_age_hours=24)
            ...
            if data is not None and not data.empty:
                # 保存到缓存
                self.cache_manager.save_to_cache(symbol, data, start_date, end_date)
        ...
        for source in available_sources:
            try:
                ...
                if source == ChinaDataSource.TUSHARE:
                    ...
                    logger.warning(f"⚠️ [股票信息] {source.value}不支持股票信息获取")
                    continue
    ...
    def _format_stock_data_response(self, data: pd.DataFrame, symbol: str, stock_name: str,
        ...
        try:
            ...
            result += f"   RSI12: {latest_data['rsi12']:.2f}\n"
            result += f"   RSI24: {latest_data['rsi24']:.2f}\n\n"

            # 布林带
            result += f"📊 布林带 (BOLL):\n"
            result += f"   上轨: ¥{latest_data['boll_ub']:.2f}\n"
            result += f"   中轨: ¥{latest_data['boll_mid']:.2f}\n"
            result += f"   下轨: ¥{latest_data['boll_lb']:.2f}\n\n"
